...
type mockSessionTrackerClient struct {
	auth.ClientI
	sessions []types.SessionTracker
}

func (m mockSessionTrackerClient) GetActiveSessionTrackers(ctx context.Context) ([]types.SessionTracker, error) {
...
func TestRootHostUsers(t *testing.T) {
	...
	t.Run("test create temporary user and close", func(t *testing.T) {
		cl := &mockSessionTrackerClient{}
		users, err := srv.NewHostUsers(context.Background(), "host_uuid", cl)
		require.NoError(t, err)
	...
	t.Run("test create sudoers enabled users", func(t *testing.T) {
		...
		t.Cleanup(func() {
			os.Remove("/etc/sudoers.d/teleport-" + testuser)
			host.UserDel(testuser)
		})
	...
	t.Run("test delete all users in teleport service group", func(t *testing.T) {
		cl := &mockSessionTrackerClient{
			sessions: []types.SessionTracker{&types.SessionTrackerV1{
				Spec: types.SessionTrackerSpecV1{
					Kind:    "ssh",
					State:   types.SessionState_SessionStatePending,
					Address: "host_uuid",
					Login:   "teleport-user1",
				},
			}},
		}
		users, err := srv.NewHostUsers(context.Background(), "host_uuid", cl)
		...
		_, _, err = users.CreateUser("teleport-user1", &services.HostUsersInfo{})
		require.NoError(t, err)

		deleteableUsers := []string{"teleport-user2", "teleport-user3"}
		...
		// user 1 not deleted as they have an active session
		_, err = user.Lookup("teleport-user1")
		require.NoError(t, err)
		...
		cl.sessions = []types.SessionTracker{}
